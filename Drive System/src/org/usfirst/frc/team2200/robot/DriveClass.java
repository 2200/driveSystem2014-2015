...
//	}
	...
	public void driveLinear(double slope, double startMotorValue, double time, double accelPerc){
		double startTime = Timer.getFPGATimestamp();
		double speedFunction;
		double posAccelTime = time * (accelPerc);
		double b = 1;
		SmartDashboard.putNumber("timer", (Timer.getFPGATimestamp() - startTime));
		SmartDashboard.putNumber("posAccelTime",posAccelTime);
		SmartDashboard.putNumber("stattime", startTime);
		SmartDashboard.putNumber("current fpga time", Timer.getFPGATimestamp());
		SmartDashboard.putBoolean("STARTED", true);
		while((Timer.getFPGATimestamp() - startTime) < posAccelTime){
			SmartDashboard.putNumber("current fpga time", Timer.getFPGATimestamp());

			SmartDashboard.putBoolean("STARTED", false);
			SmartDashboard.putBoolean("WHILE", true);
			speedFunction = ((Timer.getFPGATimestamp() - startTime) * slope) + startMotorValue;
			smiteDrive.drive(speedFunction, 0);
			Timer.delay(0.005);
			b = speedFunction;
		}
		SmartDashboard.putBoolean("WHILE", false);
		SmartDashboard.putBoolean("AFTER", true);
		//b = 1.781
		while((Timer.getFPGATimestamp() - startTime) < time){
//			speedFunction = ((Timer.getFPGATimestamp() - startTime) * slope * (-1)) + b;
//			smiteDrive.drive(speedFunction, 0);
//			Timer.delay(0.005);
		}
		smiteDrive.drive(-0.2, 0);
		Timer.delay(0.01);
		smiteDrive.drive(0, 0);
	}
	...
	//r = 0.67
	//depth = 0.095
	//a per circle = 1.4102 in^2
	//v per circle = 0.1339 in^3
	//density alluminum = 2.7g / cm^3 , .0975lb / in^3
	//weight per circle = 0.013055 lb
	//
