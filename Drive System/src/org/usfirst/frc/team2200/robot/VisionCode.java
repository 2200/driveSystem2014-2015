...
//import org.usfirst.frc.team2200.robot.Robot.ParticleReport;
//import org.usfirst.frc.team2200.robot.Robot.Scores;
...
//        SmartDashboard.putNumber("session", session);
		...
		session = NIVision.IMAQdxOpenCamera(camIdent, NIVision.IMAQdxCameraControlMode.CameraControlModeController);
...
//		CameraServer.getInstance().setImage(frame);
		...
		NIVision.imaqColorThreshold(binaryFrame2, frame, 255, NIVision.ColorMode.HSV, TOTE_HUE_RANGE, TOTE_SAT_RANGE, TOTE_VAL_RANGE);
	
		//Send particle count to dashboard
		int numParticles = NIVision.imaqCountParticles(binaryFrame2, 1);
...
//		//Send masked image to dashboard to assist in tweaking mask.
		...
		imaqError = NIVision.imaqParticleFilter4(binaryFrame, binaryFrame2, criteria, filterOptions, null);
		...
		if(numParticles > 0){
			...
			for(int particleIndex = 0; particleIndex < numParticles; particleIndex++){
				...
				particles.add(par);
				xLeft = par.BoundingRectLeft;
				xRight = par.BoundingRectRight;
			}
//			particles.sort(null);
		...
		SmartDashboard.putString("status: ", "before delay");
		if(firstRun){
			Timer.delay(timeDelay);
			firstRun = false;
		}
		SmartDashboard.putString("status: ", "after delay");
//		Timer.delay(0.005);	// wait for a motor update time
		if (scores.LongAspect > 80){
			fakeAspect = 80;
		}
		else{
			fakeAspect = scores.LongAspect;
		}
		angle = 98*(Math.acos((fakeAspect)/80));
		SmartDashboard.putNumber("Tote Angle:", angle);
